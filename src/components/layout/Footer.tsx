...
const Footer = () => {
  ...
  const socialLinks = [
    settings.facebookUrl && { url: settings.facebookUrl, Icon: Facebook, label: "Facebook" },
    settings.messengerUrl && { url: settings.messengerUrl, Icon: MessageCircle, label: "Messenger" },
    settings.youtubeUrl && { url: settings.youtubeUrl, Icon: Youtube, label: "YouTube" },
    ...
    <footer className="bg-gradient-to-br from-[hsl(213,50%,13%)] to-[hsl(213,56%,8%)] text-white">
      <div className="container mx-auto px-4 py-14">
        <div className="grid md:grid-cols-3 gap-10">
          <div>
            ...
            <p className="text-white/50 text-sm leading-relaxed mb-5">
              {language === "bn" ? settings.footerAboutBn : settings.footerAboutEn}
            </p>
            ...
            <ul className="space-y-2.5 text-sm">
              {[
                ...
                <li key={item.link}>
                  <a
                    href={item.link}
                    className="text-white/50 hover:text-accent transition-colors duration-200 inline-flex items-center gap-1.5"
                  >
            ...
            <ul className="space-y-3 text-sm">
              <li>
                ...
                >
                  ...
                  <span className="font-english">+88 {settings.phone}</span>
                </a>
              </li>
              <li>
                <a
                  href={`https://wa.me/88${settings.phone2}?text=${encodeURIComponent("Hi SB Mobile Shop! I'm reaching out from your website.")}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2.5 text-white/50 hover:text-accent transition-colors"
                >
                  <MessageCircle className="h-4 w-4 text-accent/70 shrink-0" />
                  <span className="font-english">+88 {settings.phone2}</span>
                  <span className="text-xs text-accent/60">(WhatsApp)</span>
                </a>
              </li>
              <li>
                <a
                ...
                >
                  ...
                  <span className="font-english">{settings.email}</span>
                </a>
              </li>
        ...
        <div className="border-t border-white/8 mt-12 pt-6 pb-[env(safe-area-inset-bottom,0px)] text-center text-sm text-white/30 flex items-center justify-center gap-1.5">
          © {new Date().getFullYear()} {settings.shopNameEn}.
